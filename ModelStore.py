...
class ModelStore(object):
	...
	def setModel(self, model, key, userId = None):
		if userId is None:
			self.persistModels[key] = model
		else:
			self.transientModels[key][userId] = model

	def getModel(self, key, userId = None):
		#send out the object of models to learning system
		if userId is None:
			return self.persistModels[key]
		else:
			transientModels = self.transientModels[key]
			if userId in transientModels:
				return transientModels[userId]
			else:
