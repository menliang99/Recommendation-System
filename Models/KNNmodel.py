...
#User Feature based KNN


#Question: can you change the KNN to itembased and use it as an online model?
#Possible answer:
# it may not be a good idea.  For KNN, you need to compute the nearest neighbors everytime an item is rated.
# while using Kmean, the item cluster is pre-computed. Everytime an item is rated, you just need to go there
# and pick up the similar items. It saves time for the online recommenditions.
...
class KNNmodel():
	...
	def provideRec(self, userId):
		...
		#what is the average score for all the similar users? NEED TEST
